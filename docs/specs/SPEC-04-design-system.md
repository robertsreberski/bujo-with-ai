...
| 19 | The monthly log gained an **arrange surface** with no prototype counterpart: a ghost 32px sliders trigger (40 on touch) in the section heading whose active state is neutral (`--bg-line`/`--fg`, never `--primary`, per DS-1) and whose accessible name appends "— filters active"; the menu reuses the composer popover family (deviation 14 metrics; vaul sheet below 680px at the 160ms override) with `menuitemradio`/`menuitemcheckbox` rows at 32px (40 on touch) that keep the surface open on selection — filters compose — capped to the popover's available height with internal scroll. The list itself defaults to active entries with closed states (done · cancelled · migrated · scheduled) folded behind a native `details` "Done & moved (N)" summary row at 36px (40 on touch, chevron rotating on open, no transition per DS-25); the heading meta reads "V of T items" while narrowed; month-log rows carry `showDate` date prefixes | the first real month produced an unreadably long flat list; the prototype's month log predates real usage and offers no filtering precedent, so the collapse keeps finished work one tap away without hiding it behind discovery |

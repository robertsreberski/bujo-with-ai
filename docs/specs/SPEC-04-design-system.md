...
| 14  | Three composer surfaces with no prototype counterpart. **Destination chip**: 12.5px/500 control at 28px (40 on touch), radius 6, 10px inline padding, hairline `--border` at rest with a muted chevron, aria-hidden `→` lead; `--bg-line`/`--fg-mid` on a screen default and the `--ai-*` family once overridden; its wrapper caps at 65% of the bar and the _name_ truncates, never the arrow. Parse chips (24px, radius 4) follow it and wrap to further lines — the context zone never truncates a fact. **Picker/help popover**: 288px (capped to the viewport less 24px), radius 8, 1px `--border`, `--shadow-menu`, menu curve at 120ms; below 680px the same content becomes a vaul sheet (surface-scoped classes — popover sizing never leaks into the sheet). **Suggestion panel**: in-flow (not portalled) listbox above the input, inset 12px, radius 8, 1px `--border`, `--shadow-menu`, 34px rows (40 on touch) at 13px `--fg-body`, active row `--bg-line`/`--fg`, 12px `--fg-mute` detail, plus a muted caption row (a sibling of the listbox, outside it) teaching the sigil's formats; the panel opens for all four sigils (`#` tags, `/` collections, `>` tomorrow, `@` upcoming round hours). **Capture help**: 28px `?` button (40 on touch) pinned top-right of the context zone; a `dl` grid of 13px `--fg-body` rows with 11.5px mono `kbd` chips on `--bg-line`, 1px `--border`, radius 4. The v1 `Shortcuts:` hint line is deleted — the sigil panels and help sheet carry its teaching job | the prototype had a single fixed capture target and an untyped input, so destination, completion, and grammar had nowhere to live       |

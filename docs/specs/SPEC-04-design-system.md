...
:root {
  ...
  --border-strong: #574D45; /* scrollbar thumb, needs-approval badge border, bullets */
  ...
  --fg-faint:   #8A7E75;  /* placeholder, disabled icons, strikethrough color */
  ...
  --ai-bg:      rgba(228,101,46,.15);
  --ai-fg:      #E8895A;
  --ai-border:  rgba(228,101,46,.32);

  /* status */
  --danger:         #F08375;
  --danger-bg-hover:#3A211C;
  --danger-border:  #5C332C;
  --ok:             #4ADE80;  /* connected status dot */

  /* overlay */
  --overlay:    rgba(0,0,0,.7);
  --shadow-menu:   0 10px 30px -10px rgba(0,0,0,.55);
  --shadow-dialog: 0 24px 60px -20px rgba(0,0,0,.7);
  --shadow-toast:  0 12px 30px -12px rgba(0,0,0,.7);
}
```
...
- DS-4 Type scale (px) and roles:
  ...
  | 14 | 500 | Proposal/migration card title |
...
- DS-8 The sparkle glyph is the assistant's mark everywhere (badge, Review
  kind chips, summary card, nav Review icon, settings). Never use it for
  non-AI meaning.
...
- DS-14 Radii: 4 (checkbox, small chips/badges) · 6 (buttons, inputs, nav
  ...
  10 (large cards: calendar, proposal, summary, index groups) · 12 (dialog)
...
- DS-16 **Inputs.** 36px, `--bg` bg, 1px `--border`, radius 6, 13.5px;
  focus: border `--primary` + `0 0 0 3px rgba(228,101,46,.2)` ring;
  placeholder `--fg-faint`; selection bg `--border`.
- DS-17 **Entry row.** Grid `18px 1fr auto`, column-gap 11, top-aligned;
  hover `--bg-hover`; bottom hairline `--bg-line`; background transition
  `.1s ease`. Checkbox 16px, radius 4, border `--border-check` →
  ...
  `--fg-mute`.
- DS-18 **Cards** (leftovers, calendar, proposal, summary, activity, index
  ...
  `--bg-line` hairlines.
- DS-19 **Badges & pills.** Kind chip (Review): 20px pill, border, sparkle +
  ...
  Mode badges (tool list): 19px, radius 4 — "needs approval" uses `--bg` +
  `--border-strong` border + `--fg` text; read modes use `--bg-line` +
  `--fg-mute`.
...
- DS-22 **Calendar cell.** 38px, radius 6, 12.5px; today: 600 weight,

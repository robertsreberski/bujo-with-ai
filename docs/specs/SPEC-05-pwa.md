...
- PWA-5 Workbox-generated SW, `skipWaiting` + `clientsClaim`, precaching the
  Vite build manifest (shell, fonts, icons). Navigations use an app-shell
  fallback so any route loads offline.
- PWA-6 Runtime strategies:

  | Request | Strategy |
  |---|---|
  | `GET /api/entries*`, `/api/collections*`, `/api/proposals*`, `/api/activity*`, `/api/summary*` | network-first, 3s timeout, cache fallback |
...
- PWA-7 Offline UX: reads come from the store mirror (SPEC-07 §4); captures
  and entry actions queue silently. A thin "Offline — changes will sync"
...
- PWA-8 Update flow: on SW update, show a quiet "Update ready — Reload"
  affordance in settings; never auto-reload mid-session.
...
- PWA-9 Safe-area variables and white-bar fix, verbatim:
  ...
  input, select, textarea { font-size: 16px; } /* iOS zoom prevention (DS-6) */
  .scrollable { overflow-y: auto; overflow-x: hidden; overscroll-behavior-y: contain; }
  ```
...
- PWA-17 The type-menu popover and dialogs opened while the keyboard is up
  position against the *visual* viewport (`--vv-*`), not the layout
  viewport.

## 5. Touch & scroll behavior

- PWA-18 Touch targets ≥40px (checkbox hit area padded beyond its 16px
  visual; calendar cells are 38px + gap — acceptable as a dense grid).
...
- PWA-23 On `visibilitychange → visible` / `pageshow` (incl. bfcache):
  ...
  `keyboard-open` state and `--vv-*` vars, reconnect SSE, flush the outbox,
  and refetch the visible day range (SPEC-07 §5). iOS suspends PWAs
  aggressively; resume must be cheap and idempotent.
- PWA-24 "Today" rollover: recompute the current date on resume and on a
  timer at local midnight; a day section for the new day appears without
  reload (relevant for an app that stays open overnight).
